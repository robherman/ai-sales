...
const parseJsonFile = async (file: { text: () => any }) => {
...
const parsePngText = (arrayBuffer: any[]) => {
  ...
  const textChunk = chunks
    ...
    .map((chunk: any) => decodeTextChunk(chunk.data))
...
const decodeTextChunk = (data: string | any[]) => {
  let i = 0;
  const keyword = [];
  const text = [];
...
const detectFormats = (json: {
  ...
  info: {
    character: { name: any; title: any; description: any; greeting: any };
  };
}) => {
  const formats = [];
